#!/usr/bin/env bash
# OS bootstrap: apt packages from packages/debian-minimal.txt (+ user adn, deploy.conf).
...
_package_list() {
  if [[ -f /etc/os-release ]]; then
    ...
    if [[ "${ID:-}" == "ubuntu" ]]; then
      printf '%s\n' "$ADN_DEPLOY_HOME/packages/ubuntu-minimal.txt"
      return
    fi
  fi
  printf '%s\n' "$ADN_DEPLOY_HOME/packages/debian-minimal.txt"
}
